...
class SSAQNAgent(agent.Agent):
    ...
    def __init__(self, train_simulators, test_simulators=None, log_folder='logs', max_words=8192):
        ...
        self.target_model_state = None#qwq
        self.target_model_action = None#qwq
        self.target_model_dot_state_action = None#qwq
    ...
    def act(self, state, actions, epsilon=0):
        ...
        return self.q_precomputed_state(state, actions, 1 ,epsilon, Boltzmann_approach=False, softmax_selection=False, penalize_history=True)
    ...
    def create_model(self, embedding_dimensions, lstm_dimensions, dense_dimensions, optimizer, embeddings=None,
        ...
        lstm_state = LSTM(lstm_dimensions, name="lstm_state")(embedding_state)
        lstm_action = LSTM(lstm_dimensions, name="lstm_action")(embedding_action)

        dense_state0 = Dense(24, activation='tanh', name="dense_state0")(lstm_state)
        dense_state1 = Dense(16, activation='tanh', name="dense_state1")(dense_state0)
        dense_state2 = Dense(dense_dimensions, activation='tanh', name="dense_state2")(dense_state1)
        dense_action = Dense(dense_dimensions, activation='tanh', name="dense_action")(lstm_action)

        model_state = Model(inputs=input_state, outputs=dense_state2, name="state")
...
######################################################################################################
        # num_words = len(self.tokenizer.word_index)
        logger.info('Creating a target model based on %s unique tokens.', num_words)

        # create the shared embedding layer (with or without pre-trained weights)
        target_embedding_shared = None

        if embeddings is None:
            target_embedding_shared = Embedding(num_words + 1, embedding_dimensions, input_length=None, mask_zero=True,
                                         trainable=embeddings_trainable, name="embedding_shared")
        else:
            logger.info('Target == Importing pre-trained word embeddings.')
            target_embeddings_index = load_embeddings(embeddings)

            # indices in word_index start with a 1, 0 is reserved for masking padded value
            target_embedding_matrix = np.zeros((num_words + 1, embedding_dimensions))
            ...
            for word, i in self.tokenizer.word_index.items():
                ...
                if embedding_vector is not None:
                    ...
                    logger.warning('Target == Word not found in embeddings: %s', word)

                    target_embedding_shared = Embedding(num_words + 1, embedding_dimensions, input_length=None, mask_zero=True,
                                                 trainable=embeddings_trainable, weights=[target_embedding_matrix],
                                                 name="embedding_shared")

        target_input_state = Input(batch_shape=(None, None), name="target_input_state")
        target_input_action = Input(batch_shape=(None, None), name="target_input_action")

        target_embedding_state = embedding_shared(target_input_state)
        target_embedding_action = embedding_shared(target_input_action)

        target_lstm_state  = LSTM(lstm_dimensions, name="target_lstm_state")(target_embedding_state)
        target_lstm_action = LSTM(lstm_dimensions, name="target_lstm_action")(target_embedding_action)

        target_dense_state0 = Dense(24, activation='tanh', name="target_dense_state0")(target_lstm_state)
        target_dense_state1 = Dense(16, activation='tanh', name="target_dense_state1")(target_dense_state0)
        target_dense_state2 = Dense(dense_dimensions, activation='tanh', name="target_dense_state2")(target_dense_state1)
        target_dense_action = Dense(dense_dimensions, activation='tanh', name="target_dense_action")(target_lstm_action)

        target_model_state = Model(inputs=target_input_state, outputs=target_dense_state2, name="target_state")
        target_model_action = Model(inputs=target_input_action, outputs=target_dense_action, name="target_action")

        self.target_model_state = target_model_state
        self.target_model_action = target_model_action


        target_input_dot_state = Input(shape=(dense_dimensions,))
        target_input_dot_action = Input(shape=(dense_dimensions,))
        target_dot_state_action = Dot(axes=-1, normalize=True, name="dot_state_action")([target_input_dot_state, target_input_dot_action])

        target_model_dot_state_action = Model(inputs=[target_input_dot_state, target_input_dot_action], outputs=target_dot_state_action,
                                       name="target_dot_state_action")
        self.target_model_dot_state_action = target_model_dot_state_action



        target_model = Model(inputs=[target_model_state.input, target_model_action.input],
                      outputs=model_dot_state_action([target_model_state.output, target_model_action.output]),
                      name="target_model")
    ...
    def train_online(self, episodes=256, batch_size=256, gamma=0.95, epsilon=1, epsilon_decay=0.99,
        ...
        for i in range(episodes):
            ...
            for b in range(batch_size):
                ...
                _, current_q = self.q_precomputed_state(state, [action],0, penalize_history=False)
                alpha = 1
                ...
                if not finished:
                    ...
                    # action_max, q_max = self.q_precomputed_state(state_next, actions_next, penalize_history=False)
                    _, q_max = self.q_precomputed_state(state_next, actions_next,0, penalize_history=False)
                    
                    # q_max, = self.q_precomputed_state(state_next, actions_max, penalize_history=False)
            ...
            self.target_model.fit(x=[states, actions], y=targets, batch_size=batch_size, epochs=1, verbose=0,
                           callbacks=callbacks)
            ...
            if ((i + 1) % checkpoint_steps) == 0:
                ...
                self.target_model.save(self.log_folder + '/' + log_prefix + file_name + '1.h5')

        return

    def q_precomputed_state(self, state, actions,model_type = 1 ,t=0, Boltzmann_approach = False, softmax_selection=False, penalize_history=False):
        """
        ...
        """
        #model_type = 1, Q,model_state;else, Q'
        if (model_type==1):
            state_dense = self.model_state.predict([state.reshape((1, len(state)))])[0]
        else:
            state_dense = self.target_model_state.predict([state.reshape((1, len(state)))])[0]
        ...
        for i in range(len(actions)):
            ...
            if (model_type == 1):
                ...
                action_dense = self.target_model_action.predict([action.reshape((1, len(action)))])[0]
                q = self.target_model_dot_state_action.predict(
                    [state_dense.reshape((1, len(state_dense))), action_dense.reshape((1, len(action_dense)))])[0][0]
        ...
        if Boltzmann_approach:
            probabilities = softmax(q_values/t)
            x = random.random()
            for i in range(len(actions)):
                if x <= probabilities[i]:
                    return i, q_values[i]
