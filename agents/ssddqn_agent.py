...
######################################################################################################
        ...
        target_lstm_shared = LSTM(lstm_dimensions, name="target_lstm_shared")
        target_lstm_state = lstm_shared(target_embedding_state)
        target_lstm_action = lstm_shared(target_embedding_action)

        target_dense_state = Dense(dense_dimensions, activation='tanh', name="target_dense_state")(target_lstm_state)
        target_dense_action = Dense(dense_dimensions, activation='tanh', name="target_dense_action")(target_lstm_action)

        target_model_state = Model(inputs=target_input_state, outputs=target_dense_state, name="target_state")
        ...
        for i in range(episodes):
            ...
            for b in range(batch_size):
                ...
                if not finished:
                    ...
                    a_max, _ = self.q_precomputed_state(state_next, actions_next,1, penalize_history=False)
                    action_max = actions_next[a_max]
                    _, q_max = self.q_precomputed_state(state_next, [action_max],0, penalize_history=False)
